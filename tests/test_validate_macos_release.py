...
class ValidateMacOSReleaseTests(unittest.TestCase):
    ...
    def test_assert_release_bundle_requires_version_json_and_plist(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with unittest.mock.patch.object(self.mod, "_detect_binary_arch", return_value="arm64"):
                self.mod._assert_release_bundle(app_dir, version="1.2.3", expected_commit="abc123", expected_arch="arm64")
    ...
    def test_assert_mounted_layout_propagates_expected_arch_to_bundle_check(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with unittest.mock.patch.object(self.mod, "_assert_release_bundle") as bundle_check:
